;var ThemifyPageBuilder, ThemifyBuilderCommon, tinyMCEPreInit;
(function($, window, document, undefined) {
	...
	ThemifyPageBuilder = {
		...
		bindEvents: function() {
			...
			/* rows */
			$body.on('click', '.themify_builder_toggle_row', this.toggleRow)
            ...
            .on('submit', '#tfb_imp_component_form', this.importRowModBuilderFormSave)
            ...
            /* save column option */
            .on('submit', '#tfb_column_settings', this.columnSaving)
				...
				if (83 == event.which && (true == event.ctrlKey || true == event.metaKey)) {
					...
					if ( currentElement != 'input' && currentElement != 'textarea' ) {
						...
						var $moduleSettings = $('#tfb_module_settings'),
							...
							$columnSettings = $('#tfb_column_settings'),
		...
		setupTooltips: function() {
			var setupBottomTooltips = function() {
				$('body').on('mouseover', '[rel=themify-tooltip-bottom]', function(e) {
					var $thisElem = $(this);

					// append custom tooltip
					$thisElem.append('<span class="themify_tooltip">' + $thisElem.attr('title') + '</span>');

					// remove native tooltip
					$thisElem.data('title', $thisElem.attr('title'));
					$thisElem.attr('title', '');
				});

				$('body').on('mouseout', '[rel=themify-tooltip-bottom]', function(e) {
					var $thisElem = $(this);

					// remove custom tooltip
					$thisElem.children('.themify_tooltip').remove();

					// put back title for native tooltips
					$thisElem.attr('title', $thisElem.data('title'));
				});
			};
		...
		optionColumn: function(e) {
			...
			var self = ThemifyPageBuilder,
				$this = $(this),
				$options = $this.closest('.themify_builder_col').children('.column-data-styling').data('styling');

			var ajax_opts = {
					data: {
						action : 'column_lightbox_options'
					}
				},
				callback = function() {
					if ( 'object' === typeof $options ) {
			...
			$this.closest('.themify_builder_col').addClass('current_selected_column');

			// highlight current selected row
			$('.themify_builder_row').removeClass('current_selected_row');
			$this.closest('.themify_builder_row').addClass('current_selected_row');

			ThemifyPageBuilder._openLightbox(ajax_opts, callback, themifyBuilder.textColumnStyling);
		},
		...
		columnSaving: function(e) {
			e.preventDefault();
			var self = ThemifyPageBuilder,
				$current_selected_column = $('.current_selected_column'),
				builder_id = $current_selected_column.closest('.themify_builder_content').data('postid'),
		...
		liveUpdateRow: function( builder_id, sendData ) {
			...
			$.ajax({
				...
				success: function( data ){
					...
					$newElems.css('visibility', 'visible').appendTo(parent).unwrap();
		...
		pasteComponentBuilder: function(event) {
			...
			switch(component) {
				case 'row':
					...
					self._highlightRow($selectedRow);
				...
				case 'module':
					...
					self._highlightModule($selectedModule);
		...
		importComponentBuilder: function(event) {
			...
			switch(component) {
				case 'row':
					...
					self._highlightRow($selectedRow);
					self._openLightbox(options, null, themifyBuilder.text_import_row_data);
                break;
				...
				case 'sub-row':
					...
					self._highlightRow($selectedRow);
					self._highlightSubRow($selectedSubRow);
					self._openLightbox(options, null, themifyBuilder.text_import_sub_row_data);
					break;
				...
				case 'module':
					...
					self._highlightModule($selectedModule);
					self._openLightbox(options, null, themifyBuilder.text_import_module_data);
                break;
			}
		},
		...
		exportComponentBuilder: function(event) {
			...
			switch(component) {
				case 'row':
					...
					self._openLightbox(options, rowCallback, themifyBuilder.text_export_row_data);
                break;
				...
				case 'sub-row':
					...
					self._openLightbox(options, subRowCallback, themifyBuilder.text_export_sub_row_data);
                break;
				...
				case 'module':
					...
					self._openLightbox(options, moduleCallback, themifyBuilder.text_export_module_data);
                break;
			}
		},
		...
		_highlightSubRow: function($subRow) {
			$('.themify_builder_sub_row').removeClass('current_selected_sub_row');
			$subRow.addClass('current_selected_sub_row');
		},

		_highlightModule: function($module) {
			$('.themify_builder_content .module_menu_front').removeClass('current_selected_module');
			$module.find('.module_menu_front').addClass('current_selected_module');
		},
